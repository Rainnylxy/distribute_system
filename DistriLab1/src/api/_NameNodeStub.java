...
public class _NameNodeStub extends org.omg.CORBA.portable.ObjectImpl implements api.NameNode
{
  ...
  public api.FileMetaData open (String filepath, int mode)
  {
            ...
            try {
                ...
                $out.write_long (mode);
                $in = _invoke ($out);
                api.FileMetaData $result = api.FileMetaDataHelper.read ($in);
                return $result;
  ...
  public String[] allocateDN (String filepath, int filesize)
  {
            org.omg.CORBA.portable.InputStream $in = null;
            try {
                org.omg.CORBA.portable.OutputStream $out = _request ("allocateDN", true);
                $out.write_string (filepath);
                $out.write_long (filesize);
                $in = _invoke ($out);
                String $result[] = api.dataBlockInfoHelper.read ($in);
                return $result;
            } catch (org.omg.CORBA.portable.ApplicationException $ex) {
                $in = $ex.getInputStream ();
                String _id = $ex.getId ();
                ...
                return allocateDN (filepath, filesize        );
            } finally {
                _releaseReply ($in);
            }
  } // allocateDN
